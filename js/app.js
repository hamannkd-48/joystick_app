...
const SKETCH_TEMPLATE = `/*
  =============================================================================
  ASSISTIVE JOYSTICK — EEPROM Dynamic Configuration
  =============================================================================
  Board:    Arduino Leonardo (must be Leonardo for HID Mouse/Keyboard)
  Joystick: 2-axis analog -> A0 (X), A1 (Y), button -> D3
  Safety:   D2 must be jumpered to GND to enable output

  Generated by Statera Web App
  =============================================================================
*/
...
bool prevMainBtn = false;
bool prevSensorStates[10] = {false};
bool overrideMouse = false;

void executeAction(uint8_t code, bool press) {
  uint8_t hidCode = 0;
  bool isMouse = false;
  switch (code) {
    case 1: hidCode = MOUSE_LEFT; isMouse = true; break;
    case 2: hidCode = MOUSE_RIGHT; isMouse = true; break;
    case 3: hidCode = MOUSE_MIDDLE; isMouse = true; break;
    case 4: hidCode = KEY_RETURN; break;
    case 5: hidCode = ' '; break;
    case 6: hidCode = KEY_ESC; break;
  }
  if (hidCode == 0) return;
  if (press) {
    if (isMouse) Mouse.press(hidCode); else Keyboard.press(hidCode);
  } else {
    if (isMouse) Mouse.release(hidCode); else Keyboard.release(hidCode);
  }
}
...
void setup() {
  ...
  for (int i = 0; i < config.sensorCount; i++) {
    pinMode(config.sensors[i].pin, INPUT_PULLUP);
  }
...
void loop() {
  ...
  Profile p = PROFILES[config.profile];
  ...
  bool enabled = (digitalRead(PIN_SAFETY) == LOW) || overrideMouse;
  if (enabled) {
    cursorAccumX += (filtX / 100.0) * config.speed;
    cursorAccumY += (filtY / 100.0) * config.speed;
    int dx = (int)cursorAccumX; int dy = (int)cursorAccumY;
    cursorAccumX -= dx; cursorAccumY -= dy;
    if (dx != 0 || dy != 0) Mouse.move(dx, dy, 0);

    bool btnNow = (digitalRead(PIN_BUTTON) == LOW);
    if (btnNow && !prevMainBtn) executeAction(config.mainButtonAction, true);
    if (!btnNow && prevMainBtn) executeAction(config.mainButtonAction, false);
    prevMainBtn = btnNow;

    for (int i = 0; i < config.sensorCount; i++) {
      bool sNow = (digitalRead(config.sensors[i].pin) == LOW);
      if (sNow && !prevSensorStates[i]) executeAction(config.sensors[i].actionCode, true);
      if (!sNow && prevSensorStates[i]) executeAction(config.sensors[i].actionCode, false);
      prevSensorStates[i] = sNow;
    }
  } else {
    if (prevMainBtn) { executeAction(config.mainButtonAction, false); prevMainBtn = false; }
    for (int i = 0; i < config.sensorCount; i++) {
      if (prevSensorStates[i]) { executeAction(config.sensors[i].actionCode, false); prevSensorStates[i] = false; }
    }
...
void handleSerial() {
  while (Serial.available()) {
    char c = Serial.read();
    if (c == 'P') { config.profile = Serial.parseInt(); EEPROM.put(0, config); }
    else if (c == 'S') { config.speed = Serial.parseFloat(); EEPROM.put(0, config); }
    else if (c == 'M') { config.mainButtonAction = Serial.parseInt(); EEPROM.put(0, config); }
    else if (c == 'C') { config.sensorCount = 0; EEPROM.put(0, config); }
    else if (c == 'A') {
      int pin = Serial.parseInt();
      char comma = Serial.read();
      int action = Serial.parseInt();
      if (config.sensorCount < 10) {
        config.sensors[config.sensorCount].pin = pin;
        config.sensors[config.sensorCount].actionCode = action;
        config.sensorCount++;
        pinMode(pin, INPUT_PULLUP);
        EEPROM.put(0, config);
      }
    }
    else if (c == '0') { calibrateCenter(1500); }
    else if (c == 'r') { calibrateRange(3000); }
    else if (c == 'm') { overrideMouse = !overrideMouse; }
  }
}
...
const app = {
  ...
  configure: {
    ...
    updateSpeed(val) {
      ...
      if (serialManager?.connected) {
        serialManager.send('S' + state.cursorSpeed.toFixed(1));
      }
      markDirty();
    },
    ...
    remapButton(name, action) {
      ...
      app.configure.syncSensorsToArduino();
      markDirty();
    },
    ...
    syncSensorsToArduino() {
      ...
      serialManager.send('M' + mainAction);
      
      // Clear sensors
      serialManager.send('C');
    ...
    compile() {
      window.app.toast('Your Arduino updates instantly! No need to compile. Use "View Base Sketch" to get the initial firmware.', 'success');
    },
